...
public class CommentModel {
    ...
    public CommentModel(Context _context) {
    ...
    private static JSONObject parseJsonDetail(JSONObject jsonObject) throws JSONException {
        ...
        comment = (Comment) comment.initWithAPIJsonData(jsonObject);

        JSONObject jsonResult = new JSONObject();
        jsonResult.put(AppHeaderkey.COMMENT, comment);

        return jsonResult;
    }

    public static void apiGetComment(Context context) {
        notificationReponseListener.requestStarted();

        final String URL = AppHeaderUrl.URL_ROOT_API + AppHeaderUrl.URL_COMMENT;
